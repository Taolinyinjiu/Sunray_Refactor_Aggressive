...
MavrosHelper::MavrosHelper(ros::NodeHandle nh, MavrosHelper_ConfigList config_list) {
    ...
    if (config_list.battery == true) {
        battery_sub_ = nh.subscribe(uav_ns + "/mavros/battery",
                                    10,
                                    &MavrosHelper::mavros_battery_callback,
                                    this);
    }
...
control_common::Mavros_Battery MavrosHelper::get_battery() {
    return mavros_battery_data_;
}
...
void MavrosHelper::mavros_battery_callback(const mavros_msgs::BatteryStatus& msg) {
    mavros_battery_data_.timestamp = msg.header.stamp;

    mavros_battery_data_.voltage = msg.voltage;
    mavros_battery_data_.current = msg.current;
    mavros_battery_data_.percent = msg.remaining;
}
...
bool MavrosHelper::is_ready() {
    ...
    if (config_cache_.battery == true) {
        mavros_battery_data_.valid = (now_time - mavros_battery_data_.timestamp) <= timeout;
        ready = ready && mavros_battery_data_.valid;
    }
