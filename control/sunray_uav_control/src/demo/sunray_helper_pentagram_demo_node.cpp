...
int main(int argc, char **argv) {
  ros::init(argc, argv, "sunray_helper_pentagram_demo_node");
  ros::NodeHandle nh;
  ros::AsyncSpinner spinner(1);
  spinner.start();

  Sunray_Helper helper(nh);
  const char *tag = "HelperPentagramDemo";
  ...
  if (!helper.set_position_velocity_block(start, sunray_demo::kNominalSpeedMps)) {
  ...
  if (!helper.set_position_velocity_list_async(pentagram) ||
      !sunray_demo::wait_for_hover_and_position(
          helper, start, sunray_demo::path_timeout_s(start, pentagram))) {
